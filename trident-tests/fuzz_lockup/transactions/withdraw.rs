...
pub fn withdraw(trident: &mut Trident, fuzz_accounts: &mut AccountAddresses, withdraw_to_recipient: bool, amount: u64) {
    ...
    let ix = WithdrawInstruction::data(data).accounts(accounts.clone()).instruction();
    ...
    assertions(trident, &accounts, amount, recipient_ata_balance_before, stream_data_ata_balance_before);
}
