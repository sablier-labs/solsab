...
pub fn initialize(trident: &mut Trident, fuzz_accounts: &mut AccountAddresses) -> Instruction {
    ...
    // Derive NFT collection ATA
    let nft_collection_ata = fuzz_accounts.nft_collection_ata.insert(
        trident,
        Some(PdaSeeds::new(
            &[treasury.as_ref(), nft_token_program_id.as_ref(), nft_collection_mint.as_ref()],
            ata_program,
        )),
    );
    ...
    InitializeInstruction::data(data).accounts(accounts).instruction()
}
