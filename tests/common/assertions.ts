...
import { accountExists } from "./base";
...
export async function assertAccountExists(account: PublicKey, accountName: string) {
  assert.isTrue(await accountExists(account), `${accountName} account does not exist when it should`);
}

export async function assertAccountNotExists(account: PublicKey, accountName: string) {
  assert.isFalse(await accountExists(account), `${accountName} account exists when it should not`);
}
...
export function assertGteBn(actual: BN, expected: BN, message?: string) {
  const defaultMessage = `Expected ${actual.toString()} to be >= to ${expected.toString()}`;
  assert.isTrue(actual.gte(expected), message ?? defaultMessage);
}
